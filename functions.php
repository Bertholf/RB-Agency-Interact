		...
		function rb_agency_interact_template_include( $template ) {
			if ( get_query_var( 'type' ) && get_query_var( 'rbgroup' ) == "models") {
				...
				if (get_query_var( 'type' ) == "profileoverview") {
					...
					return dirname(__FILE__) . '/theme/custom-member-register.php'; 
...
// Handle Emails
			...
			if($rbagency_use_s2member == true){
				if(!empty($rbagency_initial_email_after_registration)){
					...
					if($rb_agency_email_can_received > 0){
						@wp_mail($rb_agency_email,  sprintf(__('[%s] New User Registration'), get_option('blogname')), $message);
					}
	...
	if ( !function_exists('wp_new_user_notification_pending') ) { 
		function wp_new_user_notification_pending( $user_id , $new_user = true) { 
				...
				if($rb_agency_email_can_received > 0){
					@wp_mail($rb_agency_email,$_subject, $message);
				}
...
function rb_get_customfields_registration(){
	...
	if(is_array($_REQUEST['profile_types'])){
			foreach($_REQUEST['profile_types'] as $k=>$v){
				$find_in_set_arr[] = " FIND_IN_SET('".$v."',customtypes.ProfileCustomDataTypeID)>0 ";
			}
	...
	foreach($results as $data){
		$showCustomfieldsGenders = get_option("ProfileCustomShowGenderArr_".$data["ProfileCustomID"],true);
		$showCustomfieldsGendersArr = explode(",",$showCustomfieldsGenders);
		if(!in_array($data["ProfileCustomID"],$arrChecker)){
			$sql = "SELECT GenderTitle FROM ".$wpdb->prefix."agency_data_gender WHERE GenderID = %d";
			$gender = $wpdb->get_row($wpdb->prepare($sql,$_REQUEST["gender"]),ARRAY_A);
			if(in_array($gender["GenderTitle"], $showCustomfieldsGendersArr) || in_array('All Gender', $showCustomfieldsGendersArr)){
...
function RbGetProfileTypeChildCheckboxAjaxRegistration($parentID,$genderID){
	...
	if($wpdb->num_rows > 0){
		foreach($childs as $child){
			...
			$allowedGenders = get_option("DataTypeID_".$child['DataTypeID'],true);
			$allowedGendersArr = explode(",",$allowedGenders);
			...
			if(in_array($genderTitle, $allowedGendersArr) || in_array("All Gender", $allowedGendersArr)){
				...
				do_action('rb_get_profile_type_childs_checkbox_ajax_register_display',$child["DataTypeID"],$genderID);
			}
			
		}
	}
}
add_action("rb_get_profile_type_childs_checkbox_ajax_register_display","RbGetProfileTypeChildCheckboxAjaxRegistration",11,2);
...
function rb_get_gender_title_by_id($id){
	global $wpdb;
	$sql = "SELECT GenderTitle FROM ".$wpdb->prefix."agency_data_gender WHERE GenderID = %d";
	$gender = $wpdb->get_row($wpdb->prepare($sql,$id),ARRAY_A);
	return $gender["GenderTitle"];
}
