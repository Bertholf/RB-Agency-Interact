			...
			if($rbagency_use_s2member == true){
				if(!empty($rbagency_initial_email_after_registration)){
					...
					if($rb_agency_email_can_received > 0){
						wp_mail($rb_agency_email,  sprintf(__('[%s] New User Registration'), get_option('blogname')), $message, $headers);
					}
...
function RbGetProfileTypeChildCheckboxAjaxRegistration($parentID){
	global $wpdb;
...
add_action("rb_get_profile_type_childs_checkbox_ajax_register_display","RbGetProfileTypeChildCheckboxAjaxRegistration",11,1);
