...
if ($profiletype == 1) { $profiletypetext = __("Agent/Producer", rb_agency_TEXTDOMAIN); } else { $profiletypetext = __("Model/Talent", rb_agency_TEXTDOMAIN); }
...
get_header();
	...
	echo "  <div id=\"content\">\n";
		...
		if (is_user_logged_in()) {
			echo "<div id=\"profile-manage\" class=\"profile-admin account clear\">\n";
			...
			if ($count > 0) {
			  while ($data = mysql_fetch_array($results)) {
				...
				if (isset($SubscriberDateExpire)) {
				  while ($data = mysql_fetch_array($results)) {
					echo "<h1>". $current_user->first_name .", ". __("enjoying your membership?", rb_agency_TEXTDOMAIN) ."</h1>";
					echo "<h3>". __("Your membership expires on ", rb_agency_TEXTDOMAIN) ." ". $SubscriberDateExpire .", renew today!</h3>";
				  } // is there record?
				} else {
					echo "<h1>". $current_user->first_name .", ". __("are you ready to get discovered?", rb_agency_TEXTDOMAIN) ."</h1>";
					echo "<h3>". __("Subscribe now, and start applying to Casting Calls, join the Talent Directory, and more.", rb_agency_TEXTDOMAIN) ."</h3>";
				}
				...
				if ($count > 0) {
				  while ($data = mysql_fetch_array($results)) {
					echo "<div id=\"subscription-rate\">";
					echo "  <div class=\"subscription-rate-title\">". $data['SubscriptionRateTitle'] ."</div>\n";
					echo "  <div class=\"subscription-rate-price\">$". $data['SubscriptionRatePrice'] ."</div>\n";
					echo "  <div class=\"subscription-rate-text\">". $data['SubscriptionRateText'] ."</div>\n";
				...
				echo "". __("Records show you are not currently linked to a model or agency profile.  Lets setup your profile now!", rb_agency_TEXTDOMAIN) ."";
			...
			echo "<p class=\"warning\">\n";
					_e('You must be logged in to edit your profile.', rb_agency_TEXTDOMAIN);
			echo "</p><!-- .warning -->\n";
			// Show Login Form
